...
public class MainActivity extends Activity implements OnClickListener {
	...
	TextView tvsongname, tvdurction,btfav,btnet,btlocal;
			...
			if (intent.getAction().equals(Constant.ACTION_UPDATE)) {
				...
				try {
					...
					if (nowbitmap != null && !nowbitmap.isRecycled()) {
						...
						list_show_album
								.setImageResource(R.drawable.yun_music_title);
					}
					if (status == 3) {
						btplay.setImageResource(R.drawable.yun_button_pause);
						isplaying = true;
					} else {
						btplay.setImageResource(R.drawable.yun_button_play);
						isplaying = false;
					}
				} catch (Exception e) {
					e.printStackTrace();
	...
	private void initView() {
		...
		/* 顶部的三个导航按钮 */
     	...
     	btlocal = (TextView) findViewById(R.id.imagemusic);
		btfav = (TextView) findViewById(R.id.imagelove);
		btnet = (TextView) findViewById(R.id.imagenet);
		/* 底部现实专辑图片 */
	...
	public void ShowNowPlayMusic() {
		...
		if (music != null) {
			...
			if (nowbitmap != null && nowbitmap.isRecycled() == false) {
				...
				list_show_album.setImageResource(R.drawable.yun_music_title);
			}
